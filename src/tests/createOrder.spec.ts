import { IKlarnaSDK, IKlarnaOrder, ICreatedKlarnaOrder } from '../@types/types';
...
describe('Tests for createOrder function', () => {
    let klarnaSDK: IKlarnaSDK;
    /** Before Each test */
    beforeEach(() => {
    ...
    it('createOrder function returns klarna order', async () => {
        const klarnaOrder: IKlarnaOrder = {
            order_lines: [
                {
        ...
        const newKlarnaOrder: ICreatedKlarnaOrder = await createOrder(
            klarnaSDK,
            klarnaOrder,
        );

        expect(newKlarnaOrder).toStrictEqual({
            order: {
                ...
                merchant_urls: {
                    ...
                    confirmation: 'http://localhost:3001/ostoskori/vahvistus',
                    push: 'http://localhost:8080/klarna/push',
                },
                html_snippet:
